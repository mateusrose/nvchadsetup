  ...
  { "nvim-neotest/nvim-nio" },
  {
    ...
    config = function()
      ...
      for name, sign in pairs(icons.dap) do
        -- Ensure `sign` is not nil and has the required elements
        local text = sign[1] or "?"
        local texthl = sign[2] or "DiagnosticInfo"
        local linehl = sign[3]
        vim.fn.sign_define(
          "Dap" .. name,
          { text = text, texthl = texthl, linehl = linehl, numhl = linehl }
        )
      end
